...
private static String FINAL_CN="yyyy/MM/dd HH:mm:ss";
	...
	public static final String PATTERN_ZN = "yyyy年MM月dd日";
	/**
	 * 
	 * description:按指定格式输出日期字符串
	 * params:TODO
	 * author:NieBin
	 * date:2016年4月15日 下午5:04:58
	 */
	public static String formatDate(Date date,String pattern){
		return new SimpleDateFormat(pattern).format(date);
	}
	
	public static String formatDateByZN(Date date){
		return new SimpleDateFormat(PATTERN_ZN).format(date);
	}
	...
	public static Date dayToDate(long ts) {
		...
		String nowDateS= DateUtils.formatDate(date, "yyyy-MM-dd");
		try {
	...
	public static long dateTimes(Date date){
		String dateS = DateUtils.formatDate(date, "yyyy-MM-dd");
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		Date dateTime = null;
		long times = 0;
		if(date !=null){
	 ...
	 * 在指定日期加或减指定类型数量
	 * @param date
	 * @param type Calendar.MONTH Calendar.DAY_OF_MONTH Calendar.DAY_OF_WEEK
	 * @param count
	 * @return
	 */
	...
	public static Date getDateLast(Date date,int day) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		cal.add(Calendar.DATE,day);
		return cal.getTime();
